...
public class DragControl : MonoBehaviour
{
    ...
    private void Update()
    {
		...
		{
            if (selectedObject != null && !Cannon.instance.isBallMoving)
			{           
                Cannon.instance.DevamTop();
            }
            selectedObject = null;
		}
        ...
        if (selectedObject != null)
        {
            Vector3 position = new Vector3(Input.mousePosition.x, Input.mousePosition.y, Camera.main.WorldToScreenPoint(selectedObject.transform.position).z);
            Vector3 worldPosition = Camera.main.ScreenToWorldPoint(position);
            selectedObject.transform.position = new Vector3(worldPosition.x, selectedObject.transform.position.y, selectedObject.transform.position.z);
